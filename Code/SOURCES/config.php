<?php
	class Config{
		...
		public static function init(): void {
			...
			//self::$passwordHash = '$2y$10$nDE7r6f9UWMbaodSEUgfD.Ei6eT7hrKImc2B.9ClMeRF20K8sIUJy';
		}
		
		public static function getKeyPath(int $type, string $code): string {
			if (!password_verify($code, self::$passwordHash)) {
				Errors::add("Code invalide", ErrorLevel::WARNING);
				return '';
			}
			return match ($type) {
				...
				default => (Errors::add("Type invalide", ErrorLevel::WARNING) ?? ''),
			};
		}

		public static function getBddInfo(int $type, ?string $code=null): string {
			...
			return match ($type) {
				1 => self::validateIndex(self::$bddHost)
					...
					: (Errors::add("Donnée manquante", ErrorLevel::WARNING) ?? ''),
